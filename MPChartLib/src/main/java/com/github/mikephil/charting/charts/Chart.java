...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected void init() {
        ...
        markerLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        markerLinePaint.setColor(Color.parseColor("#ffa200"));
        markerLinePaint.setStyle(Paint.Style.FILL_AND_STROKE);
        markerLinePaint.setStrokeWidth(4f);
    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (this instanceof BarLineChartBase) {//条形图marker
                //画线
                canvas.drawLine(pos[0], pos[1], pos[0], Utils.convertDpToPixel(80f), markerLinePaint);
                //画三角形
                Path mpath = new Path();//三角形路径
                mpath.moveTo(pos[0], Utils.convertDpToPixel(80f) + 16f);
                mpath.lineTo(pos[0] + 12f, Utils.convertDpToPixel(80f));
                mpath.lineTo(pos[0] - 12f, Utils.convertDpToPixel(80f));
                mpath.close();
                canvas.drawPath(mpath, markerLinePaint);
                //画线
                canvas.drawLine(pos[0], mViewPortHandler.contentBottom(), pos[0], mViewPortHandler.contentBottom() + 23f, markerLinePaint);

                // draw the marker
                mMarker.draw(canvas, pos[0], Utils.convertDpToPixel(80f));
            } else {
